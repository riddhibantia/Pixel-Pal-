...
/**
 * Periodic agent monitor. WorkManager intervals are scheduling hints, not exact
 * timers; the worker re-checks `enabled` and bails when polling is off.
 */
@HiltWorker
class AgentStatusWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val config = agentConfigRepository.getConfigDirect(companionId) ?: return Result.success()
